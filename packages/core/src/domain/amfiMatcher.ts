 ...
 *
 * Ported from src/features/investment/amfiMatcher.ts. The pure scoring functions
 * (extractVariant, normalizeForSearch, calculateSimilarity, findBestMatch) are
 * verbatim. autoMatchAmfiCodes was repo-injected: instead of running raw SQL
 * against expo-sqlite, it takes a SchemeRepo and uses getUnmatchedSchemes() +
 * updateAmfiCode().
 */
 ...
 */
export function extractVariant(name: string): { plan: PlanType; dist: DistType } {
 ...
 *
 * Repo-injected: the original ran `SELECT ... WHERE amfi_code IS NULL` against
 * expo-sqlite and called updateSchemeAmfiCode. Here those are delegated to the
 * SchemeRepo (getUnmatchedSchemes + updateAmfiCode), keeping this module free of
 * any SQL/Drizzle dependency.
 *
 * NOTE: force=true is intentionally NOT supported through the repo (there is no
 * bulk "clear all amfi_code" method, and bulk mutation belongs in the repo
 * layer). It is accepted for signature parity but ignored.
 */
export async function autoMatchAmfiCodes(
  deps: { schemeRepo: SchemeRepo },
  options?: { dryRun?: boolean; force?: boolean }
): Promise<{ matched: number; total: number; matches: Array<{ schemeName: string; amfiCode: string }> }> {
  const { schemeRepo } = deps;

  const unmatched = schemeRepo.getUnmatchedSchemes();
  ...
  for (const scheme of unmatched) {
    const amfiCode = await findBestMatch(scheme.schemeName, scheme.amcName);

    if (amfiCode) {
      matches.push({ schemeName: scheme.schemeName, amfiCode });

      if (!options?.dryRun) {
        schemeRepo.updateAmfiCode(scheme.id, amfiCode);
