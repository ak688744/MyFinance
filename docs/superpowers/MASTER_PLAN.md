...
**Status:** L0 + L1 + L1.5 merged to `main` (PR #1, #2, #4); design spike merged (PR #3); L2 (Web UI) + AI-categorization + AI-Settings merged. **L3 (MCP Server) build complete** (PR #12). **L4 write-tools slice built** on branch `layer/4-mcp-write-tools` — 22 composable mutation tools in `packages/mcp` (preview-gated deletes, zero core change; mcp 64 green; Groww 6/6 unchanged). Next = L4 Wealth Manager (Claude Agent SDK orchestrator).
...
| **L4** | AI / Wealth Manager | Chat-with-your-data, then specialist agents + orchestrator (Claude Agent SDK) | 🟡 In progress — write-tools slice done on `layer/4-mcp-write-tools`; orchestrator not started | [write-tools spec](specs/2026-07-16-l4-mcp-write-tools-design.md) · [plan](plans/2026-07-17-l4-mcp-write-tools.md) |
...
- [x] **L3 (MCP Server) BUILD COMPLETE 2026-07-14 — T1, branch `layer/3-mcp` (off `main`), READY TO MERGE.** Spec [specs/2026-07-11-l3-mcp-server-design.md](specs/2026-07-11-l3-mcp-server-design.md), plan [plans/2026-07-11-l3-mcp-server.md](plans/2026-07-11-l3-mcp-server.md). Built via `superpowers:subagent-driven-development` (14 tasks, fresh implementer + task review each, final opus whole-branch review). **Delivered — new `packages/mcp`:** a read-only, **stdio**, **direct-to-core** MCP server exposing **10 task-shaped tools** for the future L4 agents. `context.ts` = single wiring point (opens sqlite via core `runMigrations`, seeds, builds all 13 repos + `NavLookup` + **injectable `marketData`** (D7) + a reserved-unused `runInTransaction` (D8)); `server.ts` `buildServer(ctx)` registers all tools; `index.ts` stdio entrypoint. Tools: `get_networth_overview`, `get_investment_portfolio` (period+account), `get_investment_returns`, `get_expense_summary`, `list_transactions`, `get_loans_overview`, `get_loan_amortization`, `list_accounts`, `search_schemes`, `get_scheme_nav`. **Two-layer validation (D6):** SDK Zod `inputSchema` = protocol layer; semantic failures (unknown liability id, market outage) → `isError`; empty data → empty-but-valid. **Unit-safety (D5):** `*Inr`/`*Percent`/`xirrFraction` (raw fraction, never scaled) field labels + per-tool descriptions. **RUNS VIA `tsx src/index.ts`** — the monorepo's run-TS-directly convention (the API uses `tsx src/server.ts`); NO compile step. **T1 GATE PASSED:** Groww golden-master **6/6 unchanged** (NET diff vs main touches ONLY `packages/mcp` — zero core change); full suite green (core 251 + agents 33 + api 69 + **mcp 26**); `tsc --build` clean across the graph; seam invariant holds (no drizzle/better-sqlite3 in `mcp/src` — only a derived sqlite *type* in context.ts); real stdio server answers `tools/list` with all 10 tools. **Final opus review** found 2 Important bugs in `investments.ts` (account filter dropped from the portfolio summary; `period` advertised-but-ignored) — both FIXED via TDD with real regression tests (period wired through core `getPortfolioSummaryForPeriod`/`getHoldingsForPeriod`, user-approved spec-faithful). **INCIDENT (resolved):** the Task-13 entrypoint subagent tried to make it runnable by compiling core to `dist` JS — rewrote imports across 37 core files + shifted core's `migrationsFolder` depth → broke 151 core tests incl. Groww; reverted entirely and switched to the `tsx` convention (core restored byte-identical). **DEFERRED (per spec §8):** write tools (seam `tools/write/` reserved + empty, `runInTransaction` built-unused — arrive in L4 behind opt-in); HTTP transport (stdio-only; `buildServer` is transport-agnostic so HTTP is a later drop-in); extra tools (categories/rules/import-history/budgets) added when an agent needs them. **KNOWN minor (accepted):** `list_transactions.count` = page size (repo `query()` has no match-total); `get_scheme_nav.latest.dateStr` always null (`getLatestNAV` returns a number only). **GOTCHA:** `corepack prepare pnpm@10.4.1` now fails (resolves the pinned 11.2.2 which crashes on Node 20.20.2) — install via `node ~/.cache/node/corepack/v1/pnpm/10.4.1/dist/pnpm.cjs install --config.manage-package-manager-versions=false`; build/test via `node_modules/.bin/{tsc,vitest}`.
  - ▶ **RESUME HERE (next session):** push `layer/4-mcp-write-tools` + open PR into `main` (`gh auth switch --user ak688744`), merge, then build the **L4 Wealth Manager orchestrator** (Claude Agent SDK consuming MCP read+write tools). Node 20.
- [x] **L4 MCP write-tools BUILT 2026-07-17 — T1, branch `layer/4-mcp-write-tools` (off `main`), READY TO MERGE.** Spec [specs/2026-07-16-l4-mcp-write-tools-design.md](specs/2026-07-16-l4-mcp-write-tools-design.md), plan [plans/2026-07-17-l4-mcp-write-tools.md](plans/2026-07-17-l4-mcp-write-tools.md). **22 composable write tools** in `packages/mcp/src/tools/write/` — transactions, categories/rules, accounts, assets, liabilities. Preview-gated deletes/wide ops (`preview()` helper); `categorize_transaction` atomic via `runInTransaction`; split recipe documented in tool descriptions + integration test. **Zero core change.** **Gates:** mcp **64** green (26 L3 + 38 new); Groww golden-master **6/6 unchanged**; seam invariant clean; diff scope `packages/mcp/` only.
