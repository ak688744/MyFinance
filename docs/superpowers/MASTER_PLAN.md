...
**Status:** L0 + L1 merged to `main` (PR #1, #2). Frontend/platform **design spike done** (PR #3, merged) → mockups + design doc feed L1.5/L2. **L1.5 (Unified Investment Model): spec + plan complete 2026-06-14** (docs on `main`, build not started) → execute the plan next as a fresh T1 cycle.
...
- [x] **L1.5 (Unified Investment Model) BRAINSTORM + PLAN COMPLETE 2026-06-14** — T1, docs-only on `main` (spec commit `da7ed7f`, plan commit `13813ee`). **NO code yet.** Spec: [specs/2026-06-14-l1.5-unified-investment-model-design.md](specs/2026-06-14-l1.5-unified-investment-model-design.md) (11 locked decisions D1–D11). Plan: [plans/2026-06-14-l1.5-unified-investment-model.md](plans/2026-06-14-l1.5-unified-investment-model.md) (30 self-contained TDD tasks). project-memory decision id `8658c1ce`.
  ...
  - ▶ **RESUME HERE (next session):** **Execute the L1.5 plan** via `superpowers:subagent-driven-development` (fresh subagent per task), branch `layer/1.5-unified-investment` off `main`, **start at Task 0**. T1 gate: existing **Groww golden-master + full core suite must pass UNCHANGED** (MF untouched) + new rollup-continuity test (MF-only `getNetWorth.totalAssets == getPortfolioSummary.totalCurrentValue`) + textbook-oracle math tests. Node 20; typecheck via `tsc --build`; push/PR via `gh auth switch --user ak688744`. After L1.5 → **L2 (Web UI)** using the design spike §3–§5 as UI contract.
