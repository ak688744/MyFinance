...
| **L1** | Ingestion + Data API | Server-side statement/investment import (MF model), **multi-platform parser registry** (Groww/HDFC now; ET Money etc. drop-in later), **multi-account** ingestion + reads, dedupe, categorize; rule CRUD; the read API for UI + agents | 🟡 In progress | [spec](specs/2026-06-12-l1-ingestion-design.md) |
...
- [ ] **In progress:** **L1 (Ingestion + Data API)** — spec at [specs/2026-06-12-l1-ingestion-design.md](specs/2026-06-12-l1-ingestion-design.md). Server-side import endpoints over ported `core` import logic, dedupe, categorize, rule CRUD, expanded read API. **Two forward-looking seams baked in:** (1) multi-platform parser registry keyed by `platform` (Groww/HDFC now; ET Money etc. are drop-in modules emitting the same normalized `ParsedData`), (2) multi-account as first-class (multiple `accountName`/`investmentApp` per asset; `GET /investments/accounts`). Enriched expense reads deferred to L2.
